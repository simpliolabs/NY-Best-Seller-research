...
export async function updateWorkspace(
  ...
  // Deep-merge nicheProfile: if the incoming update includes nicheProfile but omits
  // culturalMap, preserve the existing culturalMap from the DB rather than wiping it.
  // This prevents UI saves (which only send flat profile fields) from overwriting the
  // culturalMap that was generated by enrichNiche or patched by a backfill script.
  let resolvedFields = fields;
  if (fields.nicheProfile) {
    const existing = await getWorkspaceById(id);
    const existingProfile = (existing?.nicheProfile ?? {}) as Record<string, unknown>;
    const incomingProfile = fields.nicheProfile as Record<string, unknown>;
    // Only preserve culturalMap if the incoming update doesn't include it
    if (!incomingProfile.culturalMap && existingProfile.culturalMap) {
      resolvedFields = {
        ...fields,
        nicheProfile: { ...incomingProfile, culturalMap: existingProfile.culturalMap } as any,
      };
    }
  }

  await db.update(workspaces).set(resolvedFields).where(eq(workspaces.id, id));
