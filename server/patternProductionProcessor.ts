    ...
    messages: [
      {
        ...
        content: [
          ...
          "STEP 1 — CAN I CONVERT IT? Set `canConvert`: almost ANY design with text, a character/animal, or a swappable object can be converted. An off-brand animal/character is NOT a reason to skip — it can be REPLACED with an on-brand mascot (see MASCOT SUBSTITUTION in STEP 3). Set canConvert=false ONLY when the design is built around an AVOID TOPIC, or there is genuinely nothing to work with (no text, no character/animal, no swappable object). Explain briefly in `fitReason`.",
          ...
          "  - MASCOT SUBSTITUTION (also use `objectSwaps`): if the main character/animal is off-brand, REPLACE it with the best-fitting on-brand mascot — keeping the SAME pose, count, composition, and art style — e.g. 'the two fighting tigers' -> 'two T-Rexes in the same circular fighting pose, each holding a pickleball paddle'. PREFER this over skipping.",
          "  - `nicheEquipment` + `subjects`: ONLY when the design has no usable text and no swappable object/character — integrate equipment into the existing subjects.",
          "",
          "HARD RULES:",
