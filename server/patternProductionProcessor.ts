    ...
    messages: [
      {
        ...
        content: [
          ...
          "     ★ THE MOST COMMON & BEST easy win is (c) the MASCOT SWAP: any funny graphic tee whose subject is an ANIMAL/CHARACTER (e.g. a cowboy frog, a raccoon, a duck) converts cleanly by swapping that animal for a KB mascot (llama, etc.) in the EXACT same pose, outfit, props, and text. The swap ALONE is NOT the finished design — it MUST unmistakably read as " + niche + " through AT LEAST ONE hook (name it in fitReason). (1) VISUAL HOOK: put the mascot IN the niche — swap activities/props/setting to niche ones (for pickleball: a rectangular paddle, a holey ball, the net, the 'KITCHEN' line, a dinking pose, a gear bag); use this for ACTIVITY/SCENE/GRID sources that ALREADY have activities/props to re-theme — NOT for a standalone plain animal (which has nothing to swap and must NOT be re-posed; that is the CLEAN-SWAP SHORTCUT). NO added text (a llama mid-dink needs no caption). (2) VERBAL HOOK: a niche pun coined from a real KB root (HARD RULE 1); use this when the source's appeal is a POSE/GAG you must KEEP (two animals hugging → keep the hug and ADD 'Came for the snacks, stayed for the dinks'). Pick the hook that fits the source; do NOT bolt text onto a design whose niche is already obvious from its props/activity, and CONVERSELY do NOT add a paddle/pose/prop/scene to a design whose niche is already carried by a number/word/pun swap — ONE hook is ENOUGH ('567.9'→'0-0-2' on the SAME skeleton needs no paddle; VELOCIREADER→VELOCIDINKER needs no extra). A faithful swap of an off-brand animal/character to an ON-BRAND KB mascot is VALID and COMPLETE on its own — the recognizable mascot IS the niche read (a standalone cute-animal tee sells on the mascot, not on a bolted-on paddle), so for a STANDALONE single subject use the CLEAN-SWAP SHORTCUT (faithful swap, NO added prop/pun). Add a niche pun/element ONLY where the source offers a clean spot for it (existing text → a pun; a held object → a paddle) OR the source's appeal is a JOKE that needs translating — never INVENT a prop (no popsicle/snack) and never re-pose to manufacture one. Set canConvert=false only if the swap target isn't a recognizable KB mascot AND there is no clean niche element to add. 'Funny graphic shirt' bestsellers are FULL of these — say TRUE generously for clean animal-swaps. Do NOT skip a clean mascot-swap for 'not pickleball enough'.",
          ...
          "",
          "  ★ CLEAN-SWAP SHORTCUT (the MOST COMMON case — a single 1:1 swap):",
          "    If the source is a standalone subject and the conversion is ONE clean swap — an off-brand",
          "    animal/character → an on-brand KB mascot, an object → a niche object, or source text → a niche",
          "    pun — do NOT write the full five-part recipe. Write ONE surgical sentence: 'Replace [the exact",
          "    source element] with [the swap], keeping the SAME art style, line-work, pose, layout, colours and",
          "    every other element pixel-for-pixel identical — change ONLY that element and add NOTHING.' The",
          "    on-brand mascot in the source's OWN faithful style and pose IS the finished design — do NOT restyle",
          "    it, re-pose it, or invent any prop (no popsicle, snack, drink, or a paddle bolted on for its own",
          "    sake). PO-flagged failure (2026-06-10): a kitten tee should have become the SAME kitten-style",
          "    raccoon in the SAME pose — instead the brain restyled it AND added a popsicle. Kill that over-edit.",
          "    Reserve the full five-part recipe + any ADDED niche prop/activity for HARDER re-themes only —",
          "    scenes, grids, or multi-element sources where a 1:1 swap genuinely cannot carry the conversion.",
          "",
          ...
          "",
          `  E. PRESERVATION LOCK — close with: "Do not change the ${product} colour, background, props, lighting, camera angle, or anything else in the scene. Beyond the planned niche hook (the mascot swap with its niche prop/activity, or the niche pun text), add NOTHING new — no drinks, wine glasses, badges, banners, frames, stars, or lifestyle decorations. Everything not explicitly swapped stays exactly as it was."`,
          "",
          ...
          "10. ADD ONLY A CLEAN NICHE ELEMENT, and only where it fits. The conversion MAY add a niche pun/text where the source has text, or (for a SCENE/GRID re-theme) niche props/activities. For a clean STANDALONE mascot swap the on-brand mascot is ENOUGH — do NOT bolt on a paddle. BANNED in all cases: anything that is not a clean niche element — wine glasses, drinks, sparkles, banners, frames, 'lifestyle' flourishes, and especially an INVENTED prop (a popsicle, a snack, or a paddle added for its own sake). PO-flagged fails: a T-Rex tee got a random wine glass; a kitten→raccoon got a popsicle — both hard FAILs. When in doubt, change LESS.",
