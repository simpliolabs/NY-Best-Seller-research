...
export async function updateProductGroup(
  id: string,
  data: Partial<Pick<InsertProductGroup, "name" | "description" | "productType" | "compareAtPrice" | "costPerItem" | "pricingTiers" | "sizeWeights" | "shopifyCategoryGid" | "categoryAttributes" | "printZone">>
): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("DB unavailable");
...
export async function updateMockupTemplate(
  id: string,
  data: Partial<Pick<InsertMockupTemplate, "colorName" | "colorHex" | "availableSizes" | "sortOrder" | "garmentBbox" | "isBestSeller">>
): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("DB unavailable");
